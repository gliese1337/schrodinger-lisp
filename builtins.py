...
basic_env = Env({
	...
	'cons':		SClos(lambda k,v,x,y:Tail(x,v, lambda vx: Tail(y,v, lambda vy: k(SList((vx,vy)))))),
	'car':		SClos(lambda k,v,x:Tail(x,v, lambda vx: k(vx.strict().value.car))),
	'cdr':		SClos(lambda k,v,x:Tail(x,v, lambda vx: k(vx.strict().value.cdr))),
	'list':		SClos(lambda k,v,*x:cps_map_eval(lambda args: k(SList(args)),v,*x)),
	'len':		SClos(lambda k,v,x:Tail(x,v,lambda vx: k(SNum(vx.strict().value.len)))),
